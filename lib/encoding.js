...
Encoding.prototype.encodeWalletTransactionKey = function(walletId, height, blockIndex) {
  ...
  if(blockIndex !== undefined) {
    var blockIndexBuffer = new Buffer(4);
    blockIndexBuffer.writeUInt32BE(blockIndex);
    buffers.push(blockIndexBuffer);
  }
...
Encoding.prototype.encodeWalletUtxoValue = function(height, satoshis, address) {
  var heightBuffer = new Buffer(4);
  heightBuffer.writeUInt32BE(height);
  var satoshisBuffer = new Buffer(8);
  satoshisBuffer.writeDoubleBE(satoshis);
  ...
  addressSizeBuffer.writeUInt8(address.length);
  var addressBuffer = new Buffer(address, 'utf8');
  return Buffer.concat([height, satoshisBuffer, addressBuffer]);
};
...
Encoding.prototype.encodeWalletUtxoSatoshisValue = function(height, address) {
  var heightBuffer = new Buffer(4);
  heightBuffer.writeUInt32BE(height);
  var addressSizeBuffer = new Buffer(1);
  addressSizeBuffer.writeUInt8(address.length);
  var addressBuffer = new Buffer(address, 'utf8');
  return Buffer.concat([height, addressBuffer]);
};
