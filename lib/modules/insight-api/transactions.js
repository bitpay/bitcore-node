...
TxController.prototype.list = function(req, res) {
  ...
  if(blockHash) {
    self.node.getBlock(blockHash, function(err, block) {
      ...
      var blockInfo = self.node.db.bitcoind.getBlockIndex(block.hash);
      var txs = self.node.db.getTransactionsFromBlock(block);
      ...
      async.mapSeries(txs, function(tx, next) {
        ...
        tx.__timestamp = block.timestamp.getTime();

        tx.populateInputs(self.node.db, [], function(err) {
          if(err) {
            return next(err);
          }
