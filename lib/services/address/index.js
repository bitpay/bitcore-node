...
AddressService.prototype.start = function(callback) {
  ...
  async.series([
    ...
    function(next) {
      ...
      self.addressTxidsDb.createIndex({address: 1, height: -1}, function(err) {
        if (err) {
          return next(err);
        }
        next();
        ...
        async.each(transaction.inputs, function processInput(input, nextInput) {
          ...
          self.prevOutsDb.findOne({_id: prevOutId}, function findPreviousOutput(err, prevOut){
            ...
            if (prevOut) {
              ...
              inputSatoshis[key] = prevOut.satoshis;
        ...
        async.forEachOf(summaryTxids, function(txids, addressKey, nextTxids) {
          async.each(txids, function(txid, nextTxid) {
            self.addressTxidsDb.insert({
              txid: txid,
              address: new Buffer(addressKey, 'hex'),
              height: block.height
            }, nextTxid);
          }, nextTxids);
        }, txidsDone);
  ...
  self.addressTxidsDb.find({
    ...
    if (!options.verbose) {
      ...
      for (var i = 0; i < docs.length; i++) {
        txids.push(docs[i].txid.buffer.toString('hex'));
      }
