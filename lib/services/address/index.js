...
AddressService.prototype.blockHandler = function(block, add, callback) {
  ...
  // Address outputs within the current block
  var addressOutputs = {};
  ...
  async.eachSeries(txs, function(tx, nextTransaction) {
    ...
    async.parallel([
      function(outputsDone) {
        async.forEachOf(tx.outputs, function(output, outputIndex, nextOutput) {
          ...
          // Keep track of address -> txid:[outputIndexes] for this block
          ...
          var addressKeyString = addressKeyBuffer.toString('hex');
          var addressValue = encoding.encodeOutputValue(outputIndex);
          if (addressOutputs[addressKeyString]) {
            addressOutputs[addressKeyString].push(addressValue);
          } else {
            addressOutputs[addressKeyString] = [addressValue];
          }
    ...
    async.forEachOf(balanceDeltas, function(delta, addressKey, nextDelta) {
      ...
      // Create batch operations to update the address outputs for this block
      for (var addressKey in addressOutputs) {
        operations.push({
          type: add ? 'put' : 'del',
          key: new Buffer(addressKey, 'hex'),
          value: Buffer.concat(addressOutputs[addressKey])
        });
      }
