 ...
 *
 * BALANCE        addressHash:type                 balance:(received?):(change?)
 *
 * TXIDS          addressHash:type                 height:txid|height:txid|height:txid
 *
 * SATOSHIS       txid:outputIndex                 addressHash:type:satoshis|txid:inputIndex:height
 *
...
AddressService.prototype.blockHandler = function(block, add, callback) {
  ...
  async.eachSeries(txs, function(tx, nextTransaction) {
    ...
    async.parallel([
      function(outputsDone) {
        async.forEachOf(tx.outputs, function(output, outputIndex, nextOutput) {
          ...
          var addressKeyBuffer = encoding.encodeOutputKey(
            addressInfo.hashBuffer,
            addressInfo.hashTypeBuffer,
            height,
            txidBuffer,
          ...
          var prevOutputValue = encoding.encodePrevOutputValue(
            addressInfo.hashBuffer,
            addressInfo.hashTypeBuffer,
            output.satoshis //TODO include script for pay-to-public-key?
          );
          if (add) {
          ...
          var outKey = encoding.encodeBalanceKey(addressInfo.hashBuffer, addressInfo.hashTypeBuffer).toString('hex');
          var outBalanceDiff = add ? output.satoshis : output.satoshis * -1;
          if (balanceDeltas[outKey]) {
            balanceDeltas[outKey] += outBalanceDiff;
          } else {
            balanceDeltas[outKey] = outBalanceDiff;
          }

          // Append this txid to address -> txids index
          var addressTxIdKey = encoding.encodeAddressTxIdKey(addressInfo.hashBuffer, addressInfo.hashTypeBuffer);
      ...
      function(inputsDone) {
        ...
        async.forEachOf(tx.inputs, function(input, inputIndex, nextInput) {
          ...
          function finish(prevOutputValue) {
            ...
            var key = encoding.encodeBalanceKey(hashBuffer, hashTypeBuffer).toString('hex');
            var balanceDiff = add ? prevOutputSatoshis * -1 : prevOutputSatoshis;
            if (balanceDeltas[key]) {
              balanceDeltas[key] += balanceDiff;
            } else {
              balanceDeltas[key] = balanceDiff;
            }
            ...
            // Append this txid to address -> txids index
            var addressTxIdKey = encoding.encodeAddressTxIdKey(hashBuffer, hashTypeBuffer);
            addressesInTransaction[addressTxIdKey.toString('hex')] = true;
      ...
      // Prepare address -> txids values for this block
      for (var addressTxIdKey in addressesInTransaction) {
        var heightAndTxId = Buffer.concat([heightBuffer, txidBuffer]);
        if (addressTxIds[addressTxIdKey]) {
          addressTxIds[addressTxIdKey] = Buffer.concat([addressTxIds[addressTxIdKey], heightAndTxId]);
        } else {
          addressTxIds[addressTxIdKey] = heightAndTxId;
        }
      }
    ...
    async.parallel([
      function(txidsDone) {
        // Create batch operations to update the txids for this address for this block
        async.forEachOf(addressTxIds, function(txids, addressTxIdKey, nextAddress) {
          var addressTxIdKeyBuffer = new Buffer(addressTxIdKey, 'hex');
          self.node.services.db.store.get(addressTxIdKeyBuffer, {
            valueEncoding: 'binary'
          }, function(err, previousTxids) {
            if (err instanceof levelup.errors.NotFoundError) {
              previousTxids = new Buffer(new Array(0));
            } else if (err) {
              return nextAddress(err);
            }
            var newValue;
            if (add) {
              newValue = Buffer.concat([previousTxids, txids]);
            } else {
              newValue = previousTxids.slice(0, previousTxids.length - txids.length);
            }
            operations.push({
              action: newValue.length ? 'put' : 'del',
              key: addressTxIdKeyBuffer,
              value: newValue
            });
            nextAddress();
          });
        ...
        // Create batch operations to update the balance for each address for this block
        async.forEachOf(balanceDeltas, function(delta, addressKey, nextDelta) {
          var addressKeyBuffer = new Buffer(addressKey, 'hex');
          self.node.services.db.store.get(addressKeyBuffer, {
            valueEncoding: 'binary'
          }, function(err, addressBalanceValue) {
            var addressBalance = new Buffer(new Array(8));
            if (err instanceof levelup.errors.NotFoundError) {
              addressBalance.writeDoubleBE(delta);
            } else if (err) {
              return nextDelta(err);
            } else {
              var balance = addressBalanceValue.readDoubleBE() + delta;
              addressBalance.writeDoubleBE(balance);
            }
            operations.push({
              type: 'put',
              key: addressKeyBuffer,
