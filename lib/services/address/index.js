  ...
  async.eachSeries(txs, function(tx, nextTransaction) {
    ...
    async.parallel([
      ...
      for (var addressTxIdKey in addressesInTransaction) {
        ...
        operations.push({
          type: add ? 'put' : 'del',
          key: Buffer.concat([addressTxIdKeyBuffer, constants.SPACER_MIN, heightAndTxId]),
          value: true
        });
      }
...
AddressService.prototype.getBalance = function(addressArg, queryMempool, callback) {
  ...
  var balanceKey = encoding.encodeBalanceKey(address.hashBuffer, hashTypeBuffer);

  this.node.services.db.store.get(balanceKey, {
    valueEncoding: 'binary'
...
AddressService.prototype.getAddressSummary = function(addressArg, options, callback) {
  ...
  // TODO: no balance option
  // TODO: no txids option
  // TODO: mempool
  ...
  this.getTransactionIds(address, options, function(err, txids) {
    if (err) {
      return callback(err);
    }
  });

  this.getBalance(address, options, function(err, balance) {
    if (err) {
      return callback(err);
    }
  });
