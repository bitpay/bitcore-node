...
AddressService.prototype.start = function(callback) {
  ...
  async.series([
    ...
    function(next) {
      self.store.createCollection('addressSummary', {
        storageEngine: {
          wiredTiger: {
    ...
    function(next) {
      self.store.createCollection('prevOuts', {
        storageEngine: {
          wiredTiger: {
            configString: 'type=lsm'
          }
    ...
    function(next) {
      self.store.createCollection('addressTxids', {
        storageEngine: {
          wiredTiger: {
            configString: 'type=lsm'
          }
    ...
    function(next) {
      self.addressSummaryDb = self.store.collection('addressSummary');
      self.prevOutsDb = self.store.collection('prevOuts');
      self.addressTxidsDb = self.store.collection('addressTxids');
      self.addressTxidsDb.createIndex({a: 1, h: -1}, function(err) {
        if (err) {
          return next(err);
        }
        next();
 ...
 *
 * The database uses the following collections and indexes:
 *
 * Collection     Document                                   Indexes
 * ------------   ---------------------------------------    --------------------
 * summary        _id: addressHash:type (BinaryData)
 *                balance: (Number)
 *                received: (Number)
 *                change: (Number)
 *                count: (Number)
 *
 * txids          txid: txid (BinaryData)                    address:height
 *                address: addressHash:type (BinaryData)
 *                height: (Number)
 *
 * outputs        _id: txid:outputIndex (BinaryData)
 *                address: addressHash:type (BinaryData)
 *                satoshis: (Number)
 *
 * unspentOuts    _id: txid:outputIndex (BinaryData)
 *                address: addressHash:type (BinaryData)
 *                height: (Number)
 *                spent: txid:inputIndex:height (BinaryData)
 *
 ...
 */
AddressService.prototype.blockHandler = function(block, add, finishBlock) {
  ...
  async.eachSeries(transactions, function processTransaction(transaction, nextTransaction) {
    ...
    async.series([
      function processInputs(inputsDone) {
        ...
        async.each(transaction.inputs, function processInput(input, nextInput) {
          ...
          if (prevOutputs[prevOutId]) {
            var addressKey = prevOutputs[prevOutId][0].toString('hex');
            var satoshis = prevOutputs[prevOutId][1];
            finish(addressKey, satoshis);
          } else {
            self.prevOutsDb.findOne({_id: prevOutId}, function findPreviousOutput(err, prevOut) {
              if (err) {
                ...
                var addressKey = prevOut.a.buffer.toString('hex');
                var satoshis = prevOut.s;
                finish(addressKey, satoshis);
              }
            });
          }
      ...
      function(outputsDone) {
        async.forEachOf(transaction.outputs, function processOutput(output, outputIndex, nextOutput) {
          ...
          var balanceDiff = add ? output.satoshis : output.satoshis * -1;

          var change = 0;
          if (inputSatoshis[key]) {
            change = Math.min(inputSatoshis[key], output.satoshis);
            inputSatoshis -= change;
          }

          if (summaryDeltas[key]) {
            summaryDeltas[key].balance += balanceDiff;
            summaryDeltas[key].received += balanceDiff;
            summaryDeltas[key].change += add ? change : change * -1;
          } else {
            summaryDeltas[key] = {
              balance: balanceDiff,
              received: balanceDiff,
              change: change
            };
          }
          ...
          prevOutputs[prevOutId] = [
            Buffer.concat([addressInfo.hashBuffer, addressInfo.hashTypeBuffer]),
            output.satoshis
          ];
          nextOutput();
    ...
    if (err) {
      return finishBlock(err);
    }

    // These bulk writes are designed to be able to be applied partially and then
    // completed with another pass or rolled back to the previous block state.

    // Operations that insert new documents should also replace an existing document if
    // it already exists. Likewise a document that is being removed should be able to be
    // removed again without error.

    // All write information that is needed to parse a block should be handled internally
    // to this block without depending on changing the state of the database.

    // In the typical sense of a database transaction, each block can be committed as a
    // "transaction" that is atomic. This is achieved with a two-phase commit (e.g.
    // https://docs.mongodb.org/manual/tutorial/perform-two-phase-commits/). We keep track
    // of the blocks that have been applied as the final action. In the event that there is an error
    // or crash while writing, we can recover and complete the write on a second pass.

    var outOperations = [];
    for (var prevOutId in prevOutputs) {
      var prevOut = prevOutputs[prevOutId];
      if (add) {
        outOperations.push({
          replaceOne: {
            filter: {
            ...
            replacement: {
              _id: prevOutId, // txid + outputIndex
              a: prevOut[0], // address
              s: prevOut[1] // script
            },
            upsert: true
    ...
    var txidOperations = [];
    for (var addressKey in summaryTxids) {
      var txids = summaryTxids[addressKey];
    ...
    var summaryOperations = [];
    for (var addressKey in summaryDeltas) {
      var delta = summaryDeltas[addressKey];
      summaryOperations.push({
        updateOne: {
          filter: {
            _id: new Buffer(addressKey, 'hex'),
            h: {
              $lt: height
            }
          },
    ...
    async.parallel([
      function(txidsDone) {
        self.addressTxidsDb.bulkWrite(txidOperations, {ordered: false}, txidsDone);
      },
      function(deltasDone) {
        self.addressSummaryDb.bulkWrite(summaryOperations, {ordered: false}, deltasDone);
      },
      function(outsDone) {
        self.prevOutsDb.bulkWrite(outOperations, {ordered: false}, outsDone);
      }
    ], finishBlock);
...
AddressService.prototype.getTransactionCount = function(addressArg, queryMempool, callback) {
  ...
  var addressKey = Buffer.concat([address.hashBuffer, hashTypeBuffer]);

  this.addressSummaryDb.findOne({
    _id: addressKey
  }, function(err, doc) {
    if (err) {
      return callback(err);
...
AddressService.prototype.getTransactionIds = function(addressArg, options, callback) {
  ...
  self.addressTxidsDb.find({
    a: {
      $in: [addressKey]
    }
  }).sort({height: -1}).toArray(function(err, docs) {
    if (err) {
      return callback(err);
    }
    if (!docs) {
    ...
    if (!options.verbose) {
      ...
      for (var i = 0; i < docs.length; i++) {
        txids.push(docs[i].t.buffer.toString('hex'));
      }
...
AddressService.prototype.getAddressSummary = function(addressArg, options, callback) {
  ...
  function getBalance(done) {
    self.getBalance(address, options, function(err, data) {
      ...
      summary.appearances = data.count;
      done();
    });
  }
