...
AddressService.prototype.start = function(callback) {
  ...
  async.series([
    ...
    function(next) {
      var store = self.node.services.db.store;
      self.addressSummaryDb = store.collection('addressSummary');
      self.prevOutsDb = store.collection('prevOuts');
      self.addressTxidsDb = store.collection('addressTxidsDb');
      ...
      function processInputs(inputsDone) {
        ...
        async.each(transaction.inputs, function processInput(input, nextInput) {
          ...
          self.prevOutsDb.findOne({_id: prevOutId}, function findPreviousOutput(err, prevOut){
            ...
            if (prevOut) {
              var key = prevOut.a.buffer.toString('hex');
      ...
      function(outputsDone) {
        async.forEachOf(transaction.outputs, function processOutput(output, outputIndex, nextOutput) {
          ...
          self.prevOutsDb.insert({
            _id: prevOutId,
            a: Buffer.concat([addressInfo.hashBuffer, addressInfo.hashTypeBuffer]),
            s: output.satoshis
          }, nextOutput);
