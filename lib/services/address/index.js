 ...
 *
 * SUMMARY        addressHash:type                 balance:received:change:transactionCount:[height:txid]
 *
...
AddressService.prototype.blockHandler = function(block, add, callback) {
  ...
  async.eachSeries(txs, function(tx, nextTransaction) {
    ...
    async.series([
      ...
      // Keep track of transaction ids for each address
      for (var addressTxIdKey in addressesInTransaction) {
        ...
        if (summaryDeltas[addressTxIdKey]) {
          summaryDeltas[addressTxIdKey].txids.push(heightAndTxid);
        } else {
          summaryDeltas[addressTxIdKey].txids = [heightAndTxid];
        }
      }
    ...
    async.forEachOf(summaryDeltas, function(delta, addressKey, nextDelta) {
      ...
      self.node.services.db.store.get(addressKeyBuffer, {
        ...
        if (err instanceof levelup.errors.NotFoundError) {
          ...
          txidBuffers = Buffer.concat(delta.txids);
        } else if (err) {
          ...
          txidBuffers = Buffer.concat([
            addressSummaryValue.slice(28, addressSummaryValue.length),
            Buffer.concat(delta.txids)
          ]);
        }

        // TODO: remove for reorg
        addressSummary = Buffer.concat([addressSummary, txidBuffers]);
...
AddressService.prototype.getTransactionIds = function(addressArg, options, callback) {
  ...
  this.node.services.db.store.get(addressKey, {
    valueEncoding: 'binary'
  }, function(err, buffer) {
    if (err instanceof levelup.errors.NotFoundError) {
      return callback(null, []);
    ...
    while(pos < buffer.length) {
      ...
      var txid = buffer.slice(pos + 4, pos + 36);
      if (options.verbose) {
        values.push([height, txid]);
      } else {
        values.push(txid.toString('hex'));
      }
      pos += 36;
    }
