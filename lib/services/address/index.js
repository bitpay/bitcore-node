...
AddressService.prototype.start = function(callback) {
  ...
  async.series([
    ...
    function(next) {
      ...
      self.addressTxidsDb = self.store.collection('addressTxidsDb');
 ...
 *                count: (Number)
 *
 * txids          _id: txid (BinaryData)                     address:height
...
AddressService.prototype.getTransactionIds = function(addressArg, options, callback) {
  ...
  var address = new Address(addressArg);
  var hashTypeBuffer = constants.HASH_TYPES_MAP[address.type];
  var addressKey = encoding.encodeSummaryKey(address.hashBuffer, hashTypeBuffer);
  ...
  self.addressTxidsDb.find({
    ...
    if (!options.verbose) {
      ...
      for (var i = 0; i < docs.length; i++) {
        txids.push(docs[i]._id.buffer.toString('hex'));
      }
