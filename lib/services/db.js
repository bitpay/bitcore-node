...
DB.prototype._checkVersion = function(callback) {
  var self = this;
  self.store.collection('blocks').find().sort({height: -1}).limit(1).toArray(function(err, doc) {
    if (err) {
      return callback(err);
    } else if (!doc.length) {
...
DB.prototype._setVersion = function(callback) {
  ...
  metaDb.insert({key: 'version', value: this.version}, callback);
};
...
DB.prototype._createIndexes = function(db, callback) {
  async.series([
    ...
    function(done) {
      db.collection('blocks').createIndex({height: -1}, {unique: true}, done);
    },
    function(done) {
      db.collection('blocks').createIndex({timestamp: 1}, done);
    }
  ], callback);
};
...
DB.prototype.start = function(callback) {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    self._createIndexes(db, function(err) {
      ...
      self.node.services.bitcoind.on('tx', this.transactionHandler.bind(this));
...
DB.prototype.loadTip = function(callback) {
  ...
  self.store.collection('blocks').find().sort({height: -1}).limit(1).toArray(function(err, blocks) {
    if (err) {
      return callback(err);
    ...
    var hash = blocks[0].hash.buffer.toString('hex');
 ...
 * @param {Function} callback
 */
DB.prototype.getBlockInfoByTimestamp = function(high, low, options, callback) {
  var self = this;
  if (!options) {
    options = {};
    callback = options;
    $.checkArgument(_.isFunction(callback), 'A callback function is expected');
  }

  // TODO: sanitize high, low

  var limit = DB.DEFAULT_BLOCK_INFO_LIMIT;
  if (options.limit) {
    if (options.limit > DB.MAX_BLOCKINFO_LIMIT) {
      return callback(new Error('Limit can not exceed the max of: ' + DB.MAX_BLOCKINFO_LIMIT));
    }
    limit = options.limit;
  }
...
DB.prototype.runAllBlockHandlers = function(block, blockBytes, add, callback) {
  ...
  async.eachSeries(
    ...
    function(err) {
      ...
      log.debug('Updating the database with new tip');
      ...
      var blockData = {
        ...
        bytes: blockBytes,
        coinbase: coinbaseScriptBuffer,
        height: block.__height
      };

      // Update block index
      if (add) {
        self.store.collection('blocks').insert(blockData);
      } else {
        self.store.collection('blocks').remove(blockData);
      }
    }
  );
};
